...
static int SpawnProjectile(EntityPool* pool, int ownerIdx, Vector3 start, Vector3 dir) {
    ...
    // Offset projectile spawn slightly ahead to avoid collision with caster
    Vector3 spawnPos = Vector3Add(start, _Vector3Scale(dir, 1.0f));

    int p = SpawnEntity(pool, spawnPos, (Vector3) { 0.25f, 0.25f, 0.25f },
    ...
    // Mark as projectile for query system
    pool->masks[p] |= COMP_IS_PROJECTILE;

    pool->meta[p].owner = ownerIdx;
    pool->meta[p].faction = pool->meta[ownerIdx].faction;

    // Initialize projectile health in the game pool
    gamePool.healthStats[p].maxHealth = 1;
    gamePool.healthStats[p].currentHealth = 1;
    gamePool.healthStats[p].armor = 0;
    gamePool.healthStats[p].magicResist = 0.0f;
    gamePool.healthStats[p].damageMultiplier = 1.0f;
...
void UpdateCombat_Query(EntityPool* pool, float dt, const int* entities, int count) {
    for (int k = 0; k < count; k++) {
        ...
        if (pool->fireCooldown[i] <= 0.0f) {
            ...
            if (melee) {
                // Use proper damage system instead of direct health manipulation
                HealthSystem_DealDamage(target, (float)pool->attackDamage[i], DAMAGE_PHYSICAL, i);
            }
            else {
    ...
    case SKILL_NOVA: {
        ...
        for (int i = 0; i < pool->count; i++) {
            ...
            if (Vector3LengthSqr(d) <= r2) {
                // Use proper damage system
                HealthSystem_DealDamage(i, 18.0f, DAMAGE_MAGICAL, idx);
            }
        }
    } break;
